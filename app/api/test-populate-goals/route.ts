...
export async function POST() {
  try {
    ...
    for (const user of testUsers) {
      ...
      const deleted = await db.delete(financialGoals).where(eq(financialGoals.userId, user.id));
      console.log(`   🗑️ Cleared existing goals for ${user.email}`);
      ...
      for (const goalData of userGoals) {
        ...
        const [createdGoal] = await db
          ...
          .values({
            ...
            targetCountry: goalData.targetCountry || 'United States',
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        totalGoalsCreated: results.length,
        totalTargetAmount: results.reduce((sum, r) => sum + parseFloat(r.amount), 0)
      }
    });
    ...
    return NextResponse.json(
      { 
        error: 'Failed to populate financial goals', 
